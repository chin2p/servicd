...
@app.post("/car_config")
def create_car_config(car_config: CarConfigCreate, user_id: int = Depends(get_current_user), conn = Depends(get_db)):
...
@app.post("/car")
def create_car(car: CarCreate, user_id: int = Depends(get_current_user), conn = Depends(get_db)):
    try:
        with conn.transaction():
            with conn.cursor() as cur:
...
def create_maintenance_type(maintenance_type: MaintenanceTypeCreate, user_id: int = Depends(get_current_user), conn = Depends(get_db)):
    with conn.transaction():
        with conn.cursor() as cur:
...
@app.post("/part")
def create_part(part: PartCreate, user_id: int = Depends(get_current_user), conn = Depends(get_db)):
...
@app.post("/service")
def create_service(service: ServiceCreate, user_id: int = Depends(get_current_user), conn = Depends(get_db)):
    with conn.cursor() as cur:
...
def create_service_part(service_part: ServicePartCreate, user_id: int = Depends(get_current_user), conn = Depends(get_db)):
...
def create_service_scheduled(service_scheduled: ServiceScheduledCreate, user_id: int = Depends(get_current_user), conn = Depends(get_db)):
    try:
        with conn.transaction():
            with conn.cursor() as cur:
...
@app.get("/cars/{car_id}")
def get_car(car_id: int, user_id: int = Depends(get_current_user), conn = Depends(get_db)):
    with conn.cursor() as cur:
...
def get_car_services(car_id: int, user_id: int = Depends(get_current_user), conn = Depends(get_db)):
...
def get_maintenance_types(conn = Depends(get_db)):
    with conn.cursor() as cur:
...
@app.get("/parts")
def get_parts(conn = Depends(get_db)):
    with conn.cursor() as cur:
...
def delete_part(service_id: int, part_id: int, user_id = Depends(get_current_user), conn = Depends(get_db)):
    with conn.cursor() as cur:
...
def delete_service(service_id: int, user_id: int = Depends(get_current_user), conn = Depends(get_db)):
    with conn.cursor() as cur:
        cur.execute("""SELECT car.user_id
                        FROM service JOIN car ON service.car_id = car.car_id
                        WHERE service.service_id = %s""", (service_id,))
...
def delete_car(car_id: int, user_id: int = Depends(get_current_user), conn = Depends(get_db)):
    with conn.cursor() as cur:
        cur.execute("SELECT user_id FROM car WHERE car_id = %s", (car_id,))
        row = cur.fetchone()
    if row is None:
...
@app.delete("/users/me")
def delete_account(user_id: int = Depends(get_current_user), conn = Depends(get_db)):
